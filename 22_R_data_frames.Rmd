...
R sees data frames as collections of columns and simplifies access to them with the dollar sign. We can check class of each column using the dollar sign shortcut. Pause the video and get class of the first row and first element in the first row - how and why do they differ?

---
## Subsetting elements in a data frame

```{r}
index <- dat$Column.1 > 1 # Which rows have numbers larger than 1
index
dat[index, ]
```

```{r}
index <- grepl("1", colnames(dat)) # Which columns have "1" in their names
index
dat[, index]
```
...
* `colnames()` (or just `names()`): returns the column names (column-centric data view)
